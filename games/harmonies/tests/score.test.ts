...
describe("score() integration", () => {
  ...
  it("scores animals and spirit from config", () => {
    // Single green = 1 tree point. Bat card (track [3,6,10,15]) with 2 cubes = track[1] = 6.
    // Owl spirit (h1 tree +3) = +3 for that bush.
    ...
    expect(points(breakdown, "animals")).toBe(6);
    expect(points(breakdown, "spirit")).toBe(3); // Owl adds 3 per h1 bush
    expect(breakdown.total).toBe(10); // 1 + 6 + 3
  });
